...
export class GetUserPlaylist extends OpenAPIRoute {
  schema = {
    tags: ["User"],
    ...
    responses: {
      "200": {
        description: "Return User Playlist",
        content: {
          "application/json": {
            schema: z.object({
              series: z.object({
      ...
      "404": {
        description: "Get user playlist not found",
        content: {
          "application/json": {
            schema: z.object({
              series: z.object({
  ...
  async handle(c) {
    ...
    try {
      ...
      const options = {
        method: "GET",
        url: c.env.URL + "/api/user/playlist",
        params: {
          secUid,
          count,
          cursor,
