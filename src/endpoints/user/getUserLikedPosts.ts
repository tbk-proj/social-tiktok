...
export class GetUserLikedPosts extends OpenAPIRoute {
  schema = {
    tags: ["User"],
    ...
    responses: {
      "200": {
        description: "Return User Liked Posts",
        content: {
          "application/json": {
            schema: z.object({
              series: z.object({
      ...
      "404": {
        description: "Get user liked posts not found",
        content: {
          "application/json": {
            schema: z.object({
              series: z.object({
  ...
  async handle(c) {
    ...
    try {
      ...
      const options = {
        method: "GET",
        url: c.env.URL + "/api/user/liked-posts",
        params: {
          secUid,
          count,
          cursor,
